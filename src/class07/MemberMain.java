 ...
 * 3. 생성된 회원 객체를 객체를 객체 배열에 담아서 출력하세요.
 * 
...
public class MemberMain {
	...
	public static void main(String[] args) {
		...
		Member[] members = new Member[98]; // member type 배열 members /선언 member 객체를 생성해서 담을 수 있게 됨.

		initData(members); // 문자열 배열에 들어있는 값으로 객체 생성해서 members에 저장해주는 메소드 호출
		...
		// 문제2. 40대 이상의 총 인원수를 구해주는 메소드 만들기
	...
	private static int getVisitCount(Member[] members) { //20대후반 여성의 총 방문횟수 start
		
		int sum = 0;
		
		for (int i = 0; i < members.length; i++) {
			if (members[i].getAge().equals("20대후반") && (members[i].getGender()=='여')) {
				sum+=members[i].getNumVisit();
			}
			
		}
		return sum;
	} // end 20대후반 여성의 총 방문횟수
	...
	private static void initData(Member[] members) {
		// 회원 데이터가 String 타입 배열에 문자열 형태로 저장되어 있다.
		...
		// 콤머를 기준으로 ㅈ ㅏ르면 -> "소영","20180419","20180615","44","여","20대후반"
		for (int i = 0; i < member.length; i++) {
			//arr[0] = "소영" arr[1] = "20180419" arr[2] = "20180615" arr[3] = "44"
			String[] arr = member[i].split(","); // 콤머를 기준으로 자르고 배열에
